...
def write_outputs(classification, recording_meta, raw_transcript_text, output_dir,
    ...
    front_matter = {
        # Governance keys for the destination repo's doc gate: every file written
        # here is a `transcript` — raw, immutable, weighted by when it happened.
        # Claude's classification label is a category, not a document type, so it
        # goes under its own key; putting it in `type:` fails the gate.
        "class": "immutable",
        "type": "transcript",
        "title": name,
        "date": date,
        "event_date": date,
        "category": classification.get("type", "other"),
...
def write_wispr_summary(wispr_info, recording_meta, output_dir, config):
    ...
    date = recording_meta.get("date", "unknown")
    # Same governance header as our own outputs (see write_outputs), minus the
    # fields that describe our summarization — this copy is Wispr's, not ours.
    fm = yaml_front_matter({
        "class": "immutable",
        "type": "transcript",
        "title": f"{title} — Wispr Flow summary",
        "date": date,
        "event_date": date,
        "source": "wispr-flow",
    })
